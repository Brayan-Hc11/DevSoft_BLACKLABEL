// Cambio de cantidad de articulos ingresado por el usuario. (``) las comillas inevertidas ya que la mayoria de teclados no las tienen
...
cartIconBtn.addEventListener('click', ()=>{
    ...
    if(lastValue === 0){
        productContainer.innerHTML = '<p class="cart-empty">Tu carrito se encuentra vacio</p>';
    }else{
        drawProductInModal();
    }
...
function deleteProduct(){
    ...
    deleteProductBtn.addEventListener('click', ()=>{
        productContainer.innerHTML = '<p class="cart-empty">Haz eliminado tus productos</p>';
        lastValue = 0;
        cartNotification.innerText = lastValue;
    });
}
...
modalthumbnails.forEach(modalthumbnail => {
    modalthumbnail.addEventListener('click', event=>{
        ...
        modalImageContainer.style.backgroundImage = `url('image-product-${event.target.id.slice(-1)}.jpg')`
    });
});
...
function drawProductInModal(){
    productContainer.innerHTML = `
        <div class="cart-modal__details-container">
            <img class="cart-modal__image" src="image-product-1-thumbnail.jpg" alt="">
            <div>
